...
const Books = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/public/books')
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          setBooks(data.books);
  ...
  const handleReadBook = (book) => {
    ...
    fetch(`http://localhost:5000/api/books/read/${book.id}`, {
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          if (verifyData.success) {
            ...
            // Now read the book
            const readRes = await fetch(`http://localhost:5000/api/books/read/${book.id}`, {
              headers: { 'Authorization': `Bearer ${studentToken}` }
            });
